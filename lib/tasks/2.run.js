...
// TODO: detect not run before (no .mafrc.js or some other esentials missing) and propose maf init
...
async function runWrapper( done ) {
  ...
  const storeConfig = mergeSettings( meta, cfg, meta, rc, sdkApps )
...
function getAppMetadata() {
  ...
  try {
    ...
    if ( e.code === `MODULE_NOT_FOUND` ) {
      throw new Error( `metadata.json not found in: ${env.cwd}/contents/` )
    } else throw e
  }
...
function mergeSettings( metadata, cfg, meta, rc, apps ) {
