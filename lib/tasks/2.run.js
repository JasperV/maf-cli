...
function startServer( storeConfig, meta, done ) {
  ...
  server.on( `message`, m => {
    if ( m.serverStart ) {
      ...
      hostile.set( server.hosts[ 0 ], `maf.sdk`, err => {
        // TODO: throw and promisify
        if ( err ) console.error( err )
      } )

      message.push( ansi.green( `http${server.secure?`s`:``}://maf.sdk:${server.port}` ) )
      ...
      message.push( `\n ${ansi.bold( `Documentation: http://localhost:${server.docsPort}` )}` )
