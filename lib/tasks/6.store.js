...
async function updateWrapper( done ) {
  ...
  try {
    if (
      ...
      const apps = storeVersion.categories.reduce(
        ( acc, categorie ) => {
          categorie.apps.forEach( app => {
            if ( !acc[ app.id ] || acc[ app.id ] !== app.version  )
            acc[ app.id ] = app.version
          } )
          return acc
        }
      , {}
      ...
      if ( apps[ identifier ] && apps[ identifier ] === `${meta.version}-${versionHash}` )  {
        spinner.succeed( `App & version already in store` )
        return done()
      } else if ( !apps[ identifier ] ) {
        spinner.fail( `App not in store` )
        return done()
      }
      ...
      console.log( `Preview your App/Store via ${ansi.yellow( `https://widgets.metrological.com/metrological/${storeHash}?direct=true` )}.` )
    }
...
function getStoreVersion( store ) {
  return new Promise( async ( resolve, reject ) => {
    try {
      ...
      storeHash = storeVersion.environmentHash
    } catch( e ) {
