...
async function compileWrapper( done ) {
  ...
  const sourceMap = sourceNode.toStringWithSourceMap( { file: files[ 0 ], sourceRoot: process.cwd() } )
  ...
  // TODO: when using sourcemaps, move prettier to package only
  const code = prettier.format( out.compiledCode || `// Your App code did not compile without errors.`, {
    tabWidth: 2
  , useTabs: false
  , parser: 'babylon'
  } )
