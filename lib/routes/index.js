...
function setApp( meta ) {
  router.get( `/apps/${meta.identifier}/Contents/${meta.scripts}`, async ( req, res ) => {
    ...
    // do it when starting the server
    // do it again on file change - update the cache

    const contentsPath = path.resolve( process.cwd(), `./Contents/` )
    const scriptPath = path.resolve( contentsPath, meta.scripts )

    const sourceNode = new SourceNode()
    include( scriptPath, contentsPath, sourceNode )

    const sourceMap = sourceNode.toStringWithSourceMap( {
      file: meta.scripts
    ...
    const out = compile( {
      jsCode: [ { src: sourceMap.code, sourceMap: sourceMap.map.toJSON(), path: scriptPath } ]
    , createSourceMap: true
    , languageIn: 'ES5'
    } )

    console.log( await formatter( out, sourceMap.map.toJSON() ) )

    // + '\n' + '//# sourceMappingURL=data:application/json;charset=utf-8;base64,' + Buffer.from( out.sourceMap ).toString( 'base64' )
    res.end( out.compiledCode )
  } )
...
function setDocs() {
  // TODO: not cache /
  // perhaps then we can use subdomain - with own cert or included cert?
  router.use( `/docs`, express.static( path.resolve( process.cwd(), `./node_modules/maf3-sdk/docs/` ) ) )
  router.use( express.static( path.resolve( process.cwd(), `./node_modules/maf3-sdk/docs/` ) ) )
}
