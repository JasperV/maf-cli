...
proxy.on( `proxyRes`, ( proxyRes, req, res ) => {
  ...
  proxyRes.on( `data`, chunk => {
    if ( !options.json ) return res.write( chunk )
    if ( size !== false ) chunk.copy( buf, i, 0, chunk.length )
    else buf.push( chunk )
    i += chunk.length
  } )

  proxyRes.on( `end`, _ => {
    if ( !options.json ) return res.end()
    if ( size === false ) buf = Buffer.concat( buf )

    if ( proxyRes.headers[ 'content-encoding' ] === `gzip` )
      zlib.gunzip( buf, ( err, data ) => {
        if ( err ) throw new Error( err.message )
        convert( data/*, proxyRes*/ )
      } )
    else convert( buf/*, proxyRes*/ )
  } )
