...
function start( env ) {
  ...
  if ( opts.version ) {
    ...
    if ( env.modulePackage && typeof env.modulePackage.version !== `undefined` )
      console.info( `Local`, ansi.blue( env.modulePackage.version )  )
  ...
  if ( !env.modulePath ) {
    ...
    console.error( ansi.red( `Try running: npm install --save-dev maf-cli` ) )
    exit( 1 )
  }
  ...
  try {
    ...
    if ( e.code === `MODULE_NOT_FOUND` ) {
      ...
      console.error( ansi.red( `Try running: npm install --save-dev Metrological/maf3-sdk` ) )
      process.exit( 1 )
    } else throw e
  }
